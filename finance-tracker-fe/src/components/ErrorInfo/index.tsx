...
export function ErrorInformation ({
  ...
  const descriptions = keepDefaultDescription ? [...DEFAULT_DESCRIPTION, ...descriptionsProp] : descriptionsProp
    ...
    <div className="text-center">
      ...
      {descriptions.map((description, i) => {
        return typeof description === 'string'
          ? <p className="my-3" key={`para-${i}`}> {description} </p>
      ...
      <div className="mt-5">
        {!!onRetryRequest && (
          ...
          >{retryRequestLabel}</button>
        )}
        {!!onRetryRequest && actionsArr.length > 0 && <div className="mt-2 text-purple-500"> OR </div>}
